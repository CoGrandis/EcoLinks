...
class PostFilesModel {
    ...
    public function getByPost($id) {
        $query = $this->conn->prepare("SELECT * FROM post_files WHERE FK_ID_POST = :post_id");
        $query->bindParam(':post_id', $id);
        $query->execute();
        return $query->fetchAll(PDO::FETCH_ASSOC);
    }
    public function create($data) { 
        $query = $this->conn->prepare("INSERT INTO post_files (FK_ID_POST, filename, filepath) VALUES (:post_id, :filename, :filepath)");
